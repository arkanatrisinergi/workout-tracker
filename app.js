...
const App = {
    ...
    async loadDashboard() {
        DOM.calendarGrid.innerHTML = `<div style="grid-column: span 7; color: #666; font-size:14px;">Loading charts...</div>`;
        
        try {
            State.historyData = await ApiService.fetchHistory();
            this.renderGrid();
        } catch (err) {
            DOM.calendarGrid.innerHTML = `<div style="grid-column: span 7; color: #da3637; font-size:12px;">Failed to load historical data grid.</div>`;
        }
    },
    ...
    renderGrid() {
        ...
        // Isolate matched dates based on local tracking structures
